...
void cancella(pnodo *grafo, tipo elem);
...
pnodo ricerca_arco(pnodo temp, tipo elem);
	...
	while (ins!='x') {
	  ...
	  switch (ins) {
        ...
        case 'S' :
                    ...
                    scanf("%s", &elem);
                    scanf("%s", &elem2);
                    while ((temp=ricerca_arco(grafo, elem))!=NULL)
                        printf("\nNodo %d puntato da %d\n", elem, temp->info);
                    break;
		case 'p' :
                    printf("\n\nInizio:");
...
/*------------------------------lcancella-----------------------------------*/
/* cancella un elemento dalla lista dei successori */
void lcancella(plista *temp, tipo elem)
{
    lista *punt;
...
/*------------------------------ricerca_arco-----------------------*/
/* Ricerca un arco diretto verso un particolare nodo, restituendo
    un puntatore al nodo che lo punta in caso di esito positvivo,
    NULL altrimenti */
pnodo ricerca_arco(pnodo temp, tipo elem)
{
    plista punt;

    if (ricerca_nodo(temp, elem)!=NULL) {
        while (temp!=NULL) {
            punt=temp->succ;
            while (punt!=NULL) {
                if (strcmp(punt->info,elem)==0) return temp;
                else punt=punt->next;
            }
            temp=temp->proxnodo;
        }
...
void cancella_nodo(pnodo *grafo, tipo elem)
{
    ...
    while ((temp=ricerca_arco(*grafo, elem))!=NULL)
        lcancella(&(temp->succ),elem);

    if ((temp=ricerca_nodo(*grafo, elem))!=NULL) {
...
/*------------------------------------cancella------------------------------*/
/* Cancella un elemento dalla lista dei nodi */
void cancella(pnodo *grafo, tipo elem)
{
    pnodo punt;
    if ((*grafo)!=NULL)
        ...
        else cancella(&((*grafo)->proxnodo), elem);
}
/*------------------------lstampa-----------------------------------*/
